...
#     try:
#       self.vd = open("/dev/v4l/by-id/usb-WITHROBOT_Inc._oCamS-1MGN-U_SN_2E955004-video-index0", 'r+b', buffering=0)
#     except:
...
class v4l2_util():
  def __init__(self, devpath, verbose=False):
    ...
    try:
      self.vd = open(devpath, 'r+b',buffering=0)
    except:
      raise IOError('Device Open Error %s'%devpath)
    if self.verbose:
      print('--Device Open Success!!!')
...
class oCams():
  def __init__(self, dev, verbose=0):
    ...
    self.Print(dev,1)
    self.cam = v4l2_util(dev)
    self.cam.card=self.cam.card.decode('utf-8')
    self.Print(self.cam.card,1)
    ...
    if self.cam.card == 'oCamS-1CGN-U':
      ...
      self.mode = 2
    elif self.cam.card == 'oCamS-1MGN-U':
      self.mode = 2
    elif self.cam.card == 'oCamStereo-K':
      self.mode = 3
    else:
